...
class LEOrderECTest extends LETestCase
{
    ...
    private function mockConnector($orderValid = false, $authValid = true, $orderStatus = null)
    {
        ...
        $connector->post("http://test.local/order/test", Argument::any())->willReturn($orderReq);
    ...
    protected function addAccountKey(FilesystemAccountStorage $store)
    {
        $public=<<<PUBLIC
-----BEGIN PUBLIC KEY-----
...
PRIVATE;
    ...
    private function mockConnectorWithNoAuths($valid = false)
    {
        ...
        $connector->post("http://test.local/order/test")->willReturn($orderReq);
